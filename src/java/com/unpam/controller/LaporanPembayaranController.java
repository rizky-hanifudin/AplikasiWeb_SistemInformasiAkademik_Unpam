 ...
 */
public class LaporanPembayaranController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!((userName == null) || userName.equals(""))) {
            ...
            if (tombol.equals("Cetak")) {
                PembayaranKuliah pembayaran = new PembayaranKuliah();


                String file = getServletConfig().getServletContext().getRealPath(".");
                System.out.println("file: "+file);
                if (pembayaran.cetakLaporan(opsi, nim, formatTypeData[noType][1], getServletConfig().getServletContext().getRealPath("reports/PembayaranReport.jrxml"))) {
                    byte[] pdfasbytes = pembayaran.getPdfasbytes();
                    try ( OutputStream outStream = response.getOutputStream()) {
                        response.setHeader("Content-Disposition", "inline; filename=PembayaranReport." + formatTypeData[noType][1]);
                    ...
                    keterangan += pembayaran.getPesan();
                }
            }

            String konten = "<h2>Mencetak Laporan Pembayaran</h2>";
            konten += "<form action='LaporanPembayaranController' method='post'>";
            konten += "<table>";
            konten += "<tr>";
